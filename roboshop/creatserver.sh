...
create_ec2() {
    ...
    echo -e "\e[35m ***** Creating Instance for ${COMPONENT} component ***** \e[0m"

    IP_ADDRESS=$(aws ec2 run-instances --image-id ${AMI_ID} --count 1 --instance-type t2.micro --security-group-ids ${SG_ID} --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=$COMPONENT}]" | jq '.Instances[].PrivateIpAddress' | sed -e 's/"//g')

    echo -e "\e[35m Private IP of  ${COMPONENT} component is: ${IP_ADDRESS}  \e[0m"
